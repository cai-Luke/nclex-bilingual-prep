...
No model-generated learner-facing clinical content becomes canonical without independent content review and the promotion pipeline. Raw model output stages in `banks-raw/`, passes validation + audit + source-check, then promotes to a canonical `banks/*.json` with a `BANK-REVIEW-LEDGER.md` entry; the generating model never reviews its own batch.
...
**16. Answer-pattern bias is presentation-layer first, content-layer only where shuffling can't reach. Status: ACTIVE (narrowed 2026-07-14 — corrects a prior self-contradiction).**
*Positional* tells (option order, dropdown index, matrix column, ordered-response scramble depth) carry no clinical meaning and are repaired mechanically by a deterministic, ID-seeded permutation. *Distributional* tells (SATA correct-count concentration, ordered-response template repetition) are properties of the item content itself and cannot be shuffled away — they are repaired only through future authoring or a deliberate targeted replacement/regeneration pass, never by hand-editing answer logic in reviewed canonical items. Incidental dilution from ordinary new content is acceptable but is **not** considered remediation: the standing global distributional FAILs are frozen debt, not self-healing, and clear only through a deliberate targeted regeneration decision.

The audit's `fix_class` encodes exactly this fork: `SHUFFLE_AT_PROMOTION` is mechanical and automatable; `REGENERATE` is a non-blocking content-design backlog item. Live thresholds (verify against code, not here): `max_cell_deviation_pp: 8`, `template_repeat_max_share: 0.15`, `scramble_min_n: 8` in `scripts/audit/non-mcq-bias-lib.ts`.
...
**Study-session distribution:** the category weight table and sampler-rule detail this section used to restate in prose now live only in code (`NCLEX_CATEGORY_WEIGHTS` in `src/schema.ts`; `sessionSampler.ts`) per principle 10 above. Full original table and calibration narrative: archived.
