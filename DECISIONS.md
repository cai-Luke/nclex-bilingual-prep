...
**Application — construction language stays off the learner surface (2026-07-21).** `Closed-world` describes an authoring construction, not wording to show a learner: the governing order, protocol, threshold, or criteria must instead be stated naturally in the question. Author/checker scaffolding such as `source-pinned`, `source-supported`, and metaphorical `lane` language is naturalized before promotion without removing the embedded rule or changing the tested construct. The finite HIGH-confidence learner-surface lexicon is enforced by [`lib/producer-vocabulary-leakage.ts`](lib/producer-vocabulary-leakage.ts); bare `lane` remains a human-review signal rather than a universal mechanical failure.
