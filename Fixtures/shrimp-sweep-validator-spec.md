...
Zero-dependency Node CLI (stdlib only — matches the static/offline, no-deps posture of the bank). Add to `package.json` scripts:

```sh
npm run validate-sweep -- manifest.jsonl summary.json
npm run validate-sweep -- manifest.jsonl summary.json --bank banks/*.json   # enables qid-existence checks
npm run validate-sweep -- manifest.jsonl summary.json --strict              # warnings also fail the run
```
...
- `fixtures/edge/*` — one fixture per hard-failure ID (a row missing a quote, a bad enum, a `replace` with `answer_key_trust: medium`, a `the_tell` absent from its quotes, counts that don't reconcile), each asserting the specific check fires.
...
2. All four fixture groups assert their expected status.
3. A clean v3 run from Gemini passes (or its failures are real and actionable).
