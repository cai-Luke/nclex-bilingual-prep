...
| **Renderers** | `src/visuals/**` | `npm run test-visuals`; kind-specific test; `selfCheck` regressions; visual smoke/parity check; `npm run validate-bank -- banks/*.json`; `npm run build`. A load-bearing arithmetic kind (`io_record`, `medication_label`, `device_screen`, `burn_map`) also needs explicit before/after `selfCheck` numeric-equality proof. Calibrated tracing geometry (`rhythm_strip`, `capnography`, `fetal_monitoring`) always needs a visual smoke. |
