...
const partial = questions.find(
  (q) => q.itemType === "select_all" && q.correct.length > 1,
);
...
async function seed(
  stores,
  settings = { languageMode: "on-tap", revisitMissed: true },
) {
  await page.evaluate(
...
async function archiveMiss(id = "primary-miss", question = q) {
  ...
  if (question.itemType === "multiple_choice")
    wrong.optionIds = [
      question.options.find((o) => !question.correct.includes(o.id)).id,
    ];
  await resume(snapshot(id, question, { answers: { [question.id]: wrong } }));
  await btn(
    question.itemType === "case_study" ? "Submit all parts" : "Submit answer",
  ).click();
  ...
  await page.evaluate(async () => {
    ...
    const tx = db.transaction(["activeSession", "completedSets"], "readwrite");
    const st = tx.objectStore("activeSession");
  ...
  await btn("Finish").evaluate((el) => {
    el.click();
    el.click();
    el.click();
  });
  await page
    .getByRole("heading", { name: "Your answers / 本次作答" })
    .waitFor();
  const archived = await read("completedSets");
  assert.equal(archived.length, 1);
  assert.equal(archived[0].sessionId, id);
  assert.equal((await read("activeSession")).length, 0);
  assert.equal(
    (await read("answerEvents")).filter((event) => event.sessionId === id)
      .length,
    1,
  );
}
try {
  ...
  await page.evaluate(async () => {
    ...
    const tx = db.transaction(
      ["progress", "answerEvents", "activeSession"],
      "readwrite",
    );
  ...
  assert.equal((await read("activeSession")).length, 0);
  pass(
    "Delayed triple submit: one durable answerEvent; captured answer and result survive crash/reload",
    {
      eventCount: 1,
      delayMs: 650,
    },
  ...
  await archiveMiss();
  pass(
    "Delayed triple Finish converges on one Last set, one answerEvent and no active record",
    { delayMs: 650 },
  );
  assert.equal(
  ...
  assert.ok(
    await page.locator(".summary-review-body .stem .chinese-line").count(),
  );
  const reviewBody = page.locator(".summary-review-body");
  const beforeFullReveal = await reviewBody.locator(".chinese-line").count();
  await reviewBody
    .getByRole("button", {
      name: "Show full Chinese 显示完整中文",
      exact: true,
    })
    .click();
  await reviewBody
    .locator(".inline-reveal")
    .first()
    .waitFor({ state: "hidden" });
  assert.equal(await reviewBody.locator(".inline-reveal").count(), 0);
  assert.ok(
    (await reviewBody.locator(".chinese-line").count()) > beforeFullReveal,
  );
  await page
  ...
  assert.ok(await term.count(), "fixture must expose contextual glossary help");
  await term.click();
  assert.ok(await page.locator(".term-popover").isVisible());
  assert.match(
    await page.locator(".term-popover").innerText(),
    /[\u3400-\u9fff]/,
  );
  assert.equal(
    (await read("answerEvents")).filter((e) => e.sessionId === "primary-miss")
      .length,
    1,
  ...
  pass(
    "Collapsed Summary fix, on-tap and reveal-all Chinese, and explicit glossary help in ambient Off survive telemetry removal",
  );
  ...
  assert.equal(
    (await read("progress")).find((p) => p.questionId === q.id).needsReview,
    false,
  );
  ...
  pass(
    "Remediation clears Needs review without replacing Last set; historical Not fully correct remains",
  );
  await page.locator(".summary-review-toggle").click();
  await page.evaluate(() =>
  ...
  await page
    .getByRole("button", {
      name: "Copy question for GPT / 复制问题给 GPT",
      exact: true,
    })
    .click();
  assert.ok(
    (await page.evaluate(() => window.__copiedQuestion)).includes(q.stem.en),
  );
  await btn("Save question").click();
  await page
    .getByRole("group", { name: "Your answers scope" })
    .getByRole("button", { name: "Saved", exact: true })
    .click();
  await btn("Remove from Saved").click();
  ...
  pass(
    "Copy-for-GPT captures the actual question; Saved-filter removal restores completed-view focus",
  );
  ...
  mismatched.entries.push({
    ...structuredClone(primary.entries[0]),
    questionId: "deleted-fixture",
  });
  await seed({ completedSets: [mismatched], activeSession: [] });
  await page.locator(".last-set-entry").click();
  await page
    .getByText("This question has changed since this set", { exact: true })
    .waitFor();
  await page
    .getByText("No longer in the current question bank", { exact: true })
    .waitFor();
  assert.equal(await page.locator(".question-card").count(), 0);
  assert.equal(await btn("Try again").count(), 1);
  pass(
    "Changed fingerprint and deleted ID degrade individually; only live IDs offered for retry",
  );
  ...
  assert.equal(
    await btn("Keep current set / 保留当前练习").evaluate(
  ...
  assert.equal(
    await start.evaluate((el) => el === document.activeElement),
    true,
  );
  ...
  pass(
    "Replacement guard preserves drafts, safe focus, Escape restoration and confirmed persistence",
  );
  ...
  const row = page
    ...
    .filter({
      has: page.getByRole("heading", { name: q.stem.en, exact: true }),
    });
  await row.click();
  await page
    .getByText("Current question preview · No previous answer is shown.")
    .waitFor();
  assert.equal((await read("activeSession")).length, 0);
  await btn("Practice this question").click();
  assert.equal((await read("activeSession"))[0].launchIntent, "remediation");
  pass(
    "Library inspection is read-only; explicit practice launches remediation",
  );
  // Partial-credit and complete whole-case UI; final initialized order is captured even without draft entry.
  await resume(
    snapshot("partial-ui", partial, {
      answers: { [partial.id]: { optionIds: [partial.correct[0]] } },
    }),
  );
  await btn("Submit answer").click();
  await page.locator(".answer-banner").waitFor();
  assert.equal(
    (await read("progress")).find((p) => p.questionId === partial.id)
      .needsReview,
    true,
  );
  await resume(
    snapshot("whole-case", caseQ, {
      answers: { [caseQ.id]: getCorrectAnswer(caseQ) },
    }),
  );
  await btn("Submit all parts").click();
  await page.locator(".answer-banner").waitFor();
  assert.equal(
    (await read("progress")).find((p) => p.questionId === caseQ.id).needsReview,
    false,
  );
  await btn("Finish").click();
  await page.locator(".summary-review-toggle").click();
  ...
  await page.locator(".answer-banner").waitFor();
  assert.deepEqual(
    (await read("activeSession"))[0].attempts[ordered.id].answer,
    getInitialAnswer(ordered),
  );
  pass(
    "Partial credit, whole-case clearing and no-draft initialized ordered-response capture in browser",
  );
  await resume(
    snapshot("matrix-keyboard", matrix, {
      answers: { [matrix.id]: getCorrectAnswer(matrix) },
    }),
  );
  await btn("Submit answer").click();
  await page.locator(".answer-banner").waitFor();
  ...
  await resumeRecovery(
    snapshot("missing", { ...q }, { questionIds: ["deleted-active"] }),
  );
  await resumeRecovery(
    snapshot("invalid-draft", q, {
      answers: { [q.id]: { optionIds: ["missing"] } },
    }),
  );
  pass(
    "Deleted active ID and invalid draft show recovery without silent deletion",
  );
  ...
  pass(
    "390×844 and desktop; both themes; 100/125/150/200% text without page overflow",
  );
  await page.evaluate(() => {
...
async function resumeRecovery(s) {
  ...
  await btn("Continue set / 继续练习").click();
  await page
    .getByRole("heading", { name: "This set needs recovery" })
    .waitFor();
