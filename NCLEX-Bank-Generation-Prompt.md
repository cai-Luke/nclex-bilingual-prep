...
- `NCLEX-Question-Schema.md` is authoritative. This portable prompt is a compact authoring aid, not a replacement for the schema doc or validator. Current authoring uses schema `2.0`; older `1.0`–`1.9` banks remain import-compatible for their supported historical shapes.
...
## SCHEMA (author at schemaVersion `2.0`; when repo access is available, confirm the current version and exact shape in `NCLEX-Question-Schema.md`)

Top-level object:
{
  "meta": { "schemaVersion": "2.0", "exam": "NCLEX-RN", "topic": "<echo TOPIC>", "category": "<echo CATEGORY>", "difficulty": "<echo DIFFICULTY>", "count": <number generated> },
  "questions": [ <Question>, ... ]
}
