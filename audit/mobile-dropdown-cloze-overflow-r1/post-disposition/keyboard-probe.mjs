...
async function measure(target, reference = false) {
  return target.evaluate((reference) => {
    ...
    nativeProbe.remove();
    const selectors = reference ? '.question-card' : '.cloze-panel, .cloze-line, .cloze-choice, .cloze-select, .cloze-token, .cloze-readout, .submit-button, .options, .matrix-panel';
...
async function focusProof() {
  ...
  await page.keyboard.press('ArrowDown'); // Open the native macOS/Chrome picker.
  await page.keyboard.press('ArrowDown'); // Move from Choose to the first option.
...
try {
 await page.goto(url); await page.locator('.study-workspace').waitFor(); await seed(witness);
 const target=selects().first();await target.focus();
 console.log('OPTIONS',await target.locator('option').allTextContents());
 for(const key of ['ArrowDown','ArrowDown','Enter','Tab','Shift+Tab','End','Tab','Shift+Tab','Home','Space','ArrowDown','Enter']) {
  await page.keyboard.press(key);
  console.log(key,await target.evaluate(e=>({value:e.value,focus:e===document.activeElement,selectedIndex:e.selectedIndex})));
 }
 await target.focus();await page.keyboard.type(dropdownOrder()[0].options[0].en.slice(0,4));await page.keyboard.press('Tab');
 console.log('typeahead',await target.inputValue());
