...
**Extension surface (design for, don't build in U4):** U6 `medication_label` and U9 `device_screen` are key-value *panels*, not grids. Add a sibling `renderFieldPanel(input)` (label/value rows) to this module when U6 lands; keep `renderDocTable` and `renderFieldPanel` colocated so the "table/form primitive" is one module. U4 ships `renderDocTable` only; add the following forward-reference comment in `table.ts` so U6/U9 extend it rather than reinvent it:
...
| `medication` names unique across rows | `duplicate_medication_name` |
...
`mar` uses the same canonical question-level `meta` block defined in **U3 spec §6.1** (now live in `NCLEX-Question-Schema.md`). It inherits all §6.1 fields and adds two `mar`-specific keyed-cue fields. Full shape:
...
"meta": {
  ...
  "tier": "strictest",
  "source": "string",
  "skill_signature": "mar:held-dose/anticoagulant-withhold",  // stable skill tag
  "stem_disambiguators": ["anticoagulant", "enoxaparin"],     // keywords tying stem to visual
  // --- mar-specific additions ---
  "keyed_cells": [
...
- Build a `DocTableInput`: columns = `["Medication","Dose","Route","Freq", ...timeGrid]`; one body row per medication. Time-slot cells show a fixed English glyph for each `MarStatus` — status tokens are always English (the stem carries Chinese context):

  | `MarStatus` | Cell token | `emphasis` |
  |---|---|---|
  | `given` | `✓` | (none) |
  | `held` | `H` | `flag` |
  | `due` | `—` | (none) |
  ...
  Empty slots (no administration entry for that time) render as a blank cell. `emphasis:"bold"` applies to the **Medication name cell only** for `isHighAlert` rows (not the whole row).
...
`invalid` (assert each `code`): `medications_empty`; `duplicate_medication_name`; `invalid_route`; `admin_time_not_in_grid`; `duplicate_administration`; `time_grid_duplicate`; `invalid_status`.
...
`lab_trend` (U3) and the table primitive + `mar` (U4) touch disjoint files; the only shared edits are the two append-only barrel lines. **U3 is now merged.** The §6.1 canonical `meta` shape is live in `NCLEX-Question-Schema.md`; `mar` content generation may proceed directly against that documented contract.
