...
You receive an English clinical case skeleton authored upstream by Opus. Convert it into one current-schema (`schemaVersion "2.0"`) Project Shrimp question-bank JSON object containing one top-level `case_study` item (and optionally one standalone `bowtie` if the skeleton includes a BOW-TIE SYNTHESIS section).
