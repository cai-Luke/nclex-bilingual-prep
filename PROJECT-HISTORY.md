...
Batch 16 is promoted with one candidate held: sixteen of nineteen staged refs across candidates
`16A`-`16D`/`16F` landed, each independently re-gated at 0 FAIL before writing (16A's `troponin_i`
...
the intervention to have succeeded). Candidate `16E-THA` (3 refs) is held: POD3's creatinine carries
a `post_intervention` tag whose causal language ("after hydration and adequate oral intake") appears
only in POD2's own sentence, not POD3's — POD3's source sentence is bare ("Creatinine 1.3 mg/dL, eGFR
42 mL/min."), identical in shape to POD1's correctly-untagged bare creatinine. The tag does not carry
forward across stages under Rule F and should be removed before resubmission.
...
the one originally flagged. `16E-THA` (3 refs) remains the only open flowsheet hold, carried forward
unchanged from Batch 16.
