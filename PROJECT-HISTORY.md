...
- **Open flowsheet holds (as of 2026-07-11):** `12G` (gallstone) is blocked on a producer-seat re-extraction using the live unitless-subclass/`inferredUnit` machinery; the refeeding baseline is blocked on the deferred multi-column temporal-panel authoring lane. Both are content holds, not schema/code blockers. `EXHIBIT-FLOWSHEET-MIGRATION-LEDGER-2026-07-05.md` is the ledger of record for both.
