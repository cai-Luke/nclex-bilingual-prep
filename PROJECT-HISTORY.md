...
the one originally flagged. No open flowsheet holds remain, though `17C-REFEEDING`'s already-promoted
`stage_60h_update` phosphate tag is flagged in `DECISIONS.md` as an open question under the Rule F
no-carry-forward ruling that closed `16E-THA` — its justification ("never stated as discontinued")
matches the pattern that ruling forecloses, and it has not yet been reviewed against it.
