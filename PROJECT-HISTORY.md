...
Current default authoring version is `2.0` (`bound` and structured-measurement `population`, PR #23, merged 2026-07-11). Schema `2.1` (explicit typed-baseline `answerableAfterStageId: {"kind":"baseline"}` boundaries) is supported and does not change the `2.0` authoring default; the four bilingual canonical banks (`claude-`, `gemini-`, `gpt-`, `hard-cases-canonical.json`) already declare `meta.schemaVersion: "2.1"` (predating the 2026-09-13 entry below; this line was stale and is corrected here, not by that work). Per-bank versions are generated in `BANK-CENSUS.md`. `NCLEX-Question-Schema.md` owns the complete feature-floor ladder; this file does not restate it.
...
**Review/Vocabulary omnibus.** Per owner disposition, `codex/review-vocab-omnibus-r1` at `e251dad` is the accepted application baseline carried into this integration. It replaces per-question SRS/Vocabulary/Review-destination machinery with a single binary `needsReview` state, adds submission idempotency and durable Last-set completion memory, and adds a bounded global remediation reservation (R1.1) to the session sampler, stopping deliberately at IndexedDB v6 pending separately authorized physical cleanup. Its own implementation receipts are at `audit/review-vocab-omnibus-r1/implementation-receipt.md` and `audit/review-vocab-omnibus-r1/r1-1/receipt.md`. This integration did not re-review the omnibus; that acceptance was supplied as a precondition. A production-tree search found no filed independent-acceptance artifact carrying the `REVIEW_VOCAB_OMNIBUS_R1_INDEPENDENTLY_ACCEPTED` sentinel that the DB-v7 work order cites — the R1.1 producer receipt itself ends `READY_FOR_REPEAT_INDEPENDENT_REVIEW`, not accepted. This gap is noted here rather than papered over; it did not block this integration, which took the omnibus baseline as owner-asserted.

**Gap closed by owner ratification (2026-09-13):** no separately filed reviewer artifact was ever located after a bounded search (`audit/known-receipt-gaps-2026-09-13/`, `audit/known-receipt-gaps-2026-09-13-independent-review/review.md`). Owner Luke Cai confirms the omnibus acceptance was made under his direction. Given the omnibus content has been live on `main` since this entry and built upon without incident by every subsequent accepted lane through the 2026-09-13 overnight-pile integration, this is recorded as the closing disposition rather than commissioning a retrospective re-review of long-shipped code.

**DB v7 physical cleanup.** Independently reviewed and accepted (Claude Sonnet 5, `review/db-v7-physical-cleanup-claude-2026-09-12-r1`, verdict at `audit/review-vocab-omnibus-r1/db-v7-physical-cleanup-r1/independent-review/verdict.md`). `DB_VERSION` moves 6→7: the four retired object stores (`flashcardProgress`, `languageMisses`, `translationRevealEvents`, `caseAnswerPartEvents`) and six retired `progress` properties (`missed`, `correctStreak`, `srsDueAt`, `srsIntervalDays`, `srsEase`, `srsLapses`) are physically deleted when present, inside the single versionchange transaction, with stop-and-escalate guards against any unexpected historical store or property and full rollback-to-intact-old-version on any injected failure. The accepted pre-v6 `needsReview` semantic migration and `completedSets` creation are preserved for users skipping directly to v7. All 13 focused deterministic scenarios and all 9 real-Chrome browser scenario groups were independently re-run (not read from producer receipts) and passed; all-bank validation, aggregate audit, TypeScript, census, and production build were unaffected. No bank, schema, grading, sampler, bilingual, or UI source changed.

**Campaign 17 Phase C.** 18 raw items (3 six-item batches; highlight/fill-in-blank/dropdown-cloze/bowtie; IV/gravity-infusion calculations, ABG interpretation, DKA/euglycemic DKA, intrapartum fetal monitoring, US-MEC contraceptive eligibility, LAM, refeeding syndrome, ECT recovery/counseling, sepsis reassessment, pediatric battery safety, sleep apnea, alcohol withdrawal) independently reviewed and promoted (Claude Sonnet 5) into `banks/gpt-canonical.json`, 779 → 797. Producer was Codex/GPT-6 on the isolated Campaign 17 worktree; frozen source and full review trail are recorded in `BANK-REVIEW-LEDGER.md`. Campaign 17 Phase A (66 residual dispositions), Phase B (75 legacy-primary survey), and Phase D (image reconnaissance) remain separate, open, unreviewed producer proposals and are **not** part of this integration.

**Integration mechanics.** The three lines were combined by cherry-picking the single Phase-C promotion commit onto the accepted-and-reviewed DB-v7 line (itself built on the accepted omnibus baseline); the cherry-pick was verified byte-identical to the original reviewed commit and touched no DB-v7 implementation file. No generated artifact was hand-merged: census/ledger state already matched after the cherry-pick since the two lines touch disjoint files, and `census:check` confirmed no drift.
