...
**Two filesystem connectors are mounted. Determine scope at runtime — never from the tool prefix.** Connector mounts drift: a host restart can silently change which roots a connector exposes. At session start, ask each connector for its allowed directories (`list_allowed_directories`) and use any connector whose allowed root contains the repository path. Both `fsmcp.lukecai.com` (the `MCP:` tools) and the second connector (the `Filesystem:` tools) have at different times reached this repo, and on 2026-07-16 both did simultaneously — so **no rule that names a specific connector as the correct one will stay true.** Do not infer that a file is missing, or that a connector cannot reach the repo, from the tool prefix or from a single access denial. If a path returns access-denied, re-check the allowed directories of every mounted connector before concluding anything. (This paragraph previously asserted that the `Filesystem:` tools could not see Project Shrimp at all. That was accurate when written and silently went stale, which is precisely why the rule is now behavioral rather than nominal — corrected 2026-07-16.) The path contains a space (`Project Shrimp`); that is harmless when the path is passed as a single argument, so do not treat the space as the cause of a miss.

**The connector's report is not the repo — suspect the reader first.** Two distinct failures have both traced to the connector rather than to disk: scope (above), and **encoding**. If a file reads back with U+FFFD, mojibake, or garbled glyphs, treat that as a **connector-read artifact until proven otherwise** — every such alarm in this project's history has been one, and repository Markdown has been independently scanned clean (2026-07-09; 2026-07-16 × 2). Do not report it as repo corruption, do not "repair" the file, and do not open a remediation lane: re-read it, or ask Luke to `grep`, before saying anything. A CI gate for this was considered and **rejected** — reasoning in `DECISIONS.md` §8; do not re-propose one. The converse applies when you *write*: your writes pass through the same connector, so ask for a plain `grep` of the final tree before governance text you authored is committed.

When you need a file — the schema, a source module, `BANK-REVIEW-LEDGER.md`, a bank JSON, an existing spec — **pull it and read it.** Do not ask Luke to paste it, and do not reconstruct its contents from memory; repo status drifts and recollection goes stale. High-value files to pull on demand:
...
The visual system is a kind registry under `src/visuals/` (the U0 refactor). The original ten roadmap kinds have landed, and U10 adds `injection_site` as kind eleven. Reusable primitives in place: `lineChart` (charts), `table.ts`'s `renderDocTable` (documentation tables) and `renderFieldPanel` / `measureFieldPanel` (key→value label/screen panels, landed in U6). The shared numeric helpers `fmt` / `fmtNum` / `roundTo` have a **single definition** in `primitives/graphPaper.ts` — every arithmetic kind imports them and none redefines (a correctness invariant, `DECISIONS.md` principle 11). The current schema version and its feature ladder are declared in `PROJECT-HISTORY.md` and `NCLEX-Question-Schema.md`. This file does not restate them. Highlight: Table remains out of scope.
